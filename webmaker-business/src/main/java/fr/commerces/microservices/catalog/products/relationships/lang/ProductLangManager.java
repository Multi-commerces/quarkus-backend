...
public class ProductLangManager {
	 ...
	 * @return
	 */
	public final Map<Long, List<ProductLangData>> findByProductIds(@NotNull final Collection<Long> productIds) {
		try (final Stream<ProductLang> streamEntity = ProductLang.findByProductIds(productIds).stream()) {
			return streamEntity
					.collect(Collectors.groupingBy(o -> o.getProduct().getId(),
							Collectors.mapping(mapper::toData, Collectors.toList())));
		}
	}
	...
	public final ProductLangCompositeData findByPK(@NotNull final Long productId,
		...
		return mapper.toCompositeData(ProductLang.findByIdProductAndLanguageCode(productId, languageCode)
	...
	public final void updateProductLang(@NotNull final LanguageCode language, @NotNull final Long productId,
		...
		ProductLang.findByIdProductAndLanguageCode(productId, language).map(pojo -> mapper.toExistingEntity(data, pojo))
