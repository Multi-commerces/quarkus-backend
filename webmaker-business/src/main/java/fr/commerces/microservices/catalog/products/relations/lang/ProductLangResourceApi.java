...
public interface ProductLangResourceApi {
	...
	summary = "Recherche les produits avec toutes les traductions", 
		description = "Retourne les informations des produits dans toutes les langues.")
	@APIResponses(value = { 
			@APIResponse(responseCode = "200", description = CODE200GET),
			@APIResponse(responseCode = "404", description = CODE404GET) 
	})
	Response getProductLangs(
			@Parameter(description = "Identifiant du produit") 
...
//	@RolesAllowed({ "gestionnaire" })
	...
	@Operation(operationId = "putProductLang", 
		...
		description = "Opération de création/modification d'un produit existant.")
	@APIResponses(value = { 
	...
	Response patchProductLang(
			 ...
			 */
			@RequestBody(content = {
					@Content(schema = @Schema(implementation = ShemaSingleData.class)) }) byte[] data);
